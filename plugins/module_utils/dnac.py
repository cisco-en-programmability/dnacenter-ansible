...
class DnacBase():
    ...
    # Network profile related common function are below
    # check_site_template, get_templates_details,
    # assign_site_to_network_profile, unassign_site_to_network_profile
    # delete_network_profiles, execute_process_task_data, value_exists
    def check_site_template(self, each_profile, profile_info):
        ...
        if onboarding_template and isinstance(onboarding_template, list)\
            and len(onboarding_template) > 0:
            profile_info["onboarding_template"] = self.get_templates_details(
                onboarding_template)

        day_n_template = each_profile.get("day_n_template")
        if day_n_template and isinstance(day_n_template, list)\
            and len(day_n_template) > 0:
            profile_info["day_n_template"] = self.get_templates_details(
                day_n_template)
        ...
        if site_name and len(site_name) > 0:
            ...
            for site in site_name:
                if site:
                    ...
                    each_site_response["site_name"] = site
