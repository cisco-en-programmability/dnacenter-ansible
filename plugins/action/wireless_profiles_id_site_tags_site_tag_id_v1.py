...
required_if = [
    ("state", "present", ["id", "siteTagId"], True),
    ("state", "absent", ["id", "siteTagId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class WirelessProfilesIdSiteTagsSiteTagIdV1(object):
    ...
    def delete_by_id_params(self):
        ...
        new_object_params['site_tag_id'] = self.new_object.get('site_tag_id')
    ...
    def get_object_by_id(self, id):
        ...
        try:
            items = self.dnac.exec(
                family="wireless",
                function="retrieve_a_specific_site_tag_for_a_wireless_profile_v1",
                params={"site_tag_id": id}
            )
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'site_tag_id', id)
        except Exception:
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get("site_tag_id")
    ...
    def update(self):
        ...
        id = id or self.new_object.get("site_tag_id")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("siteTagId")
            if id_:
                self.new_object.update(dict(site_tag_id=id_))
        result = self.dnac.exec(
            family="wireless",
            function="update_a_specific_site_tag_for_a_wireless_profile_v1",
    ...
    def delete(self):
        ...
        result = self.dnac.exec(
            family="wireless",
            function="delete_a_specific_site_tag_from_a_wireless_profile_v1",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = WirelessProfilesIdSiteTagsSiteTagIdV1(self._task.args, dnac)
