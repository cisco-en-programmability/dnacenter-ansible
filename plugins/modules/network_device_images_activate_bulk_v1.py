...
DOCUMENTATION = r"""
---
module: network_device_images_activate_bulk_v1
short_description: Resource module for Network Device Images Activate Bulk V1
description:
  - Manage operation create of the resource Network Device Images Activate Bulk V1.
  - >
    This API initiates the process of updating the software image on the given network
    devices. Providing value for
    the `installedImages` in request payload will initiate both distribution and activation
    of the images. At the end
    of this process, only the images which are part of `installedImages` will be running
    on the network devices. To
    monitor the progress and completion of the update task, call the GET API
    `/dna/intent/api/v1/networkDeviceImageUpdates?parentId={taskId}`, where `taskId`
    is from the response of the
    current endpoint.
...
EXAMPLES = r"""
- name: Create
  cisco.dnac.network_device_images_activate_bulk_v1:
    dnac_host: "{{dnac_host}}"
