    ...
    suboptions:
      ip_address:
        description:
          ...
          - The "hostname" or "ip_address" of the network device and "fabric_site_name_hierarchy" are required for performing port assignment or port channel
            operations.
...
class SDAHostPortOnboarding(DnacBase):
    ...
    def get_delete_port_assignments_params(self, port_assignment_details, network_device_id, fabric_id):
        ...
        if not port_assignment_details:
            ...
            #     "network_device_id": network_device_id,
            # }
            # delete_port_assignments_params_list = [delete_port_assignments_params]
            # self.log("No port_assignment_details provided. Handling 'delete all'. Params: {0}"
                    ...
                    for existing_field, req_field in comparison_fields:
                        ...
                        if req_field == "protocol":
                            ...
                            if update_protocol != existing_value:
                                self.log("Protocol update not allowed. Exiting.", "ERROR")
                                self.msg = (
                        ...
                        if req_field == "connected_device_type":
                            if existing_value == "TRUNK" and req_value == "EXTENDED_NODE" and existing_channel.get("protocol") != "PAGP":
                                self.log("Connected device type change not allowed. Exiting.", "ERROR")
                                self.msg = (
    ...
    def get_delete_port_channels_params(self, port_channel_details, get_port_channels_params):
        ...
        if not existing_port_channel_details:
            self.log("No existing port channels found. Hence delete port channels operation is not required.", "INFO")
            return results
        ...
        if not port_channel_details:
            ...
            self.log("Deleting all port assignments with params: {}".format(get_port_channels_params), "INFO")
            results[0] = {
        ...
        # Track matches for requested channels
        matched_indices = set()
    ...
    def verify_delete_port_assignments_requirement(self, delete_port_assignments_params_list, get_port_assignments_params):
        ...
        if not delete_port_assignments_params_list:
            ...
            if not existing_port_assignments:
                self.log("No existing port assignments found. Hence port assignments deletion operation not required.", "INFO")
                return results
        ...
        for index, delete_port_assignment_param in enumerate(delete_port_assignments_params_list):
            ...
            if delete_required:
                ...
                self.log(
                    "Port assignment: {0} does not exist in the Cisco Catalyst Center hence delete not required."
                    .format(delete_port_assignment_param)
                )
