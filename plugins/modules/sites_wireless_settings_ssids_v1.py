...
DOCUMENTATION = r"""
---
module: sites_wireless_settings_ssids_v1
short_description: Resource module for Sites Wireless Settings Ssids V1
description:
  - Manage operations create, update and delete of the resource Sites Wireless Settings
    Ssids V1.
  - This API allows the user to create an SSID Service Set Identifier at the Global
    site.
  - >
    This API allows the user to delete an SSID Service Set Identifier at the global
    level , if the SSID is not mapped
    to any Wireless Profile, Or remove override from given site Id .
  - This API allows the user to update an SSID Service Set Identifier at the given
    site.
...
options:
  ...
  aclName:
    ...
    type: str
  authServer:
    description: For Guest SSIDs ('wlanType' is 'Guest' and 'l3AuthType' is 'web_auth'),
      the Authentication Server('authServer') is mandatory. Otherwise, it defaults
      to 'auth_external'.
    type: str
  authServers:
  ...
  basicServiceSetClientIdleTimeout:
    description: This refers to the duration of inactivity, measured in seconds, before
      ...
      Default is Basic ServiceSet ClientIdle Timeout if exists else 300. If it needs
  ...
  isRadiusProfilingEnabled:
    description: '''true'' if Radius profiling needs to be enabled, defaults to ''false''
      if not specified. At least one AAA/PSN server is required to enable Radius Profiling.'
    type: bool
  isRandomMacFilterEnabled:
    description: Deny clients using randomized MAC addresses when set to true.
    type: bool
  l3AuthType:
    description: "L3 Authentication Type. When 'wlanType' is 'Enterprise', 'l3AuthType'
      is optional and defaults to 'open' if not specified. If 'wlanType' is 'Guest'
      then 'l3AuthType' is mandatory."
    type: str
  ...
  passphrase:
    ...
    type: str
  policyProfileName:
    description: "Policy Profile Name. If 'policyProfileName' is not provided, the
      value of 'profileName' will be assigned to it. If 'profileName' is also not
      provided, an autogenerated name will be used. Autogenerated name is generated
      by appending 'ssid’ field’s value with '_profile’ (Example If 'ssid’ = 'ExampleSsid’,
      then autogenerated name will be 'ExampleSsid_profile’)."
    type: str
  profileName:
