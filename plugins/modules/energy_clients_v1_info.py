...
DOCUMENTATION = r"""
---
module: energy_clients_v1_info
short_description: Information module for Energy Clients V1
description:
  - Get all Energy Clients V1.
  - >
    Retrieves a list of client devices with energy data based on the specified query
...
options:
  ...
  cursor:
    description:
      - >
        Cursor query parameter. It's an opaque string field that indicates the next
        record in the requested
        collection. If no records remain, the API returns a response with a count
        of zero. The default value is an
        empty string, and the initial value must be an empty string. The cursor value
        is populated by the API in the
        response page block. If the user wants more records, the cursor in the subsequent
        request must be updated
  ...
  id:
    description:
      - >
        Id query parameter. The list of Mac addresses (e.g., `54 9F C6 43 FF 80`).
        Examples `id=54 9F C6 43 FF 80`
        (single device requested) `id=54 9F C6 43 FF 80&id=01 23 45 67 89 AB`.
    type: str
  siteId:
    description:
      - >
  ...
  deviceCategory:
    description:
      - >
        ...
        (multiple device
        categories with comma separator).
    type: str
  deviceSubCategory:
    description:
      - >
        DeviceSubCategory query parameter. The list of device sub categories. Examples
        `deviceSubCategory=IP Phone
        7821` (single sub category requested) `deviceSubCategory=IP Phone 7821&deviceSubCategory=IEEE
        PD`.
    type: str
  view:
    description:
      - >
        View query parameter. List of views. View and attribute work in union. Each
        view will include its
        attributes. For example, view device includes all the attributes related to
        device. Please refer to
        `ClientDeviceEnergyView` model for supported list of views Examples `view=device&view=energy`.
    type: str
  attribute:
    description:
      - >
        Attribute query parameter. List of attributes. Please refer to `ClientDeviceEnergyAttribute`
        for supported
        list of attributes Examples `attribute=id&attribute=energyConsumed`.
    type: str
requirements:
  - dnacentersdk >= 2.4.9
  - python >= 3.5
seealso:
  - name: Cisco DNA Center documentation for Clients GetClientsEnergyV1
    description: Complete reference of the GetClientsEnergyV1 API.
    link: https://developer.cisco.com/docs/dna-center/#!get-clients-energy
notes:
  - SDK Method used are clients.Clients.get_clients_energy_v1,
  - Paths used are get /dna/data/api/v1/energy/clients,
"""
EXAMPLES = r"""
- name: Get all Energy Clients V1
  cisco.dnac.energy_clients_v1_info:
    dnac_host: "{{dnac_host}}"
