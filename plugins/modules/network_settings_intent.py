...
class DnacNetwork(DnacBase):
    ...
    def get_have_global_pool(self, config):
        ...
        if (global_pool_settings := config.get("GlobalPoolDetails").get("settings")) is None:
            self.msg = "settings in GlobalPoolDetails is missing in the playbook"
            self.status = "failed"
            return self

        if (global_pool_ippool := global_pool_settings.get("ippool")) is None:
            self.msg = "ippool in GlobalPoolDetails is missing in the playbook"
            self.status = "failed"
            return self

        if (name := global_pool_ippool[0].get("ipPoolName")) is None:
        ...
        if global_pool.get("exists") == False and \
            (prev_name := global_pool_ippool[0].get("prev_name")) is not None:
    ...
    def get_have_reserve_pool(self, config):
        ...
        if (name := reserve_pool_details.get("name")) is None:
        ...
        if reserve_pool.get("exists") == False and \
            (prev_name := reserve_pool_details.get("prev_name")) is not None:
    ...
    def get_want_network(self, network_management_details):
        ...
        if network_management_details.get("timezone") is not None:
            ...
            want_network_settings["timezone"] = \
                self.have.get("network").get("net_details").get("timezone")
        ...
        if (snmpServer := network_management_details.get("snmpServer")):
