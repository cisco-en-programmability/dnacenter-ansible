...
description:
  - Get all Network Applications. - > Retrieves the list of network applications along with experience and health metrics.
    ...
    document - https //github.com/cisco-en-programmability/catalyst-center-api- specs/blob/main/Assurance/CE_Cat_Center_Org-NetworkApplications-1.0.0-resolved.yaml.
...
options:
  ...
  applicationName:
    description:
      - >
        ApplicationName query parameter. Name of the application for which the experience data is intended.
        Examples `applicationName=webex` (single applicationName requested)
  ...
  attribute:
    description:
      - >
        ...
        of the response along with the required attributes. Supported attributes are applicationName, siteId,
        exporterIpAddress, exporterNetworkDeviceId, healthScore, businessRelevance, usage, throughput,
        packetLossPercent, networkLatency, applicationServerLatency, clientNetworkLatency, serverNetworkLatency,
        trafficClass, jitter, ssid Examples `attribute=healthScore` (single attribute requested)
...
requirements:
  ...
  - python >= 3.12
seealso:
  - name: Cisco DNA Center documentation for Applications RetrievesTheListOfNetworkApplicationsAlongWithExperienceAndHealthMetrics
    description: Complete reference of the RetrievesTheListOfNetworkApplicationsAlongWithExperienceAndHealthMetrics API.
