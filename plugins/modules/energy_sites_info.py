...
short_description: Information module for Energy Sites Info
description:
  - This module represents an alias of the module energy_sites_v1_info
...
options:
  ...
  siteName:
    description:
      - >
        ...
        (single siteName
        requested) `?siteName=building1&siteName=building2&siteName=building3` (multiple
        siteNames requested).
    type: str
  siteType:
    description:
  ...
  deviceCategory:
    description:
      - >
        DeviceCategory query parameter. The list of device categories. Note that this
        filter specifies which devices
        will be included when calculating energy consumption values, rather than specifying
        the list of returned
        sites. Examples `deviceCategory=Switch` (single device category requested)
        `deviceCategory=Switch&deviceCategory=Router` (multiple device categories
  ...
  views:
    description:
      - >
        Views query parameter. The specific summary view being requested. This is
        an optional parameter which can be
        ...
        **Energy** energyConsumed, estimatedCost, estimatedEmission, carbonIntensity,
        numberOfDevices When this
        query parameter is not added the default summaries are **site,energy** Examples
        views=site (single view
        requested) views=site,energy (multiple views requested).
    type: str
  attribute:
    description:
      - >
        ...
        siteType, latitude,
        longitude, energyConsumed, estimatedCost, estimatedEmission, carbonIntensity,
        numberOfDevices If length of
        attribute list is too long, please use 'view' param instead. Examples attribute=siteHierarchy
        (single
        attribute requested) attribute=siteHierarchy&attribute=energyConsumed (multiple
...
requirements:
  ...
  - python >= 3.5
seealso:
  - name: Cisco DNA Center documentation for Sites GetSitesEnergyV1
    description: Complete reference of the GetSitesEnergyV1 API.
    link: https://developer.cisco.com/docs/dna-center/#!get-sites-energy
notes:
  - SDK Method used are sites.Sites.get_sites_energy_v1,
  - Paths used are get /dna/data/api/v1/energy/sites,
  - It should be noted that this module is an alias of energy_sites_v1_info
"""
EXAMPLES = r"""
- name: Get all Energy Sites Info
