...
description:
  ...
  - Get Energy Sites by id. - > Retrieve the energy summary data for a specific site based on the site ID. For detailed information
    ...
    sitesEnergy-1.0.1-resolved.yaml. - > Retrieves a list of sites with energy data based on the specified query parameters.
    ...
    programmability/catalyst-center-api-specs/blob/main/Assurance/CE_Cat_Center_Org- sitesEnergy-1.0.1-resolved.yaml.
  ...
  siteName:
    description:
      - >
        ...
        (`*`) character search support. E.g. `*San*, *San, San*` Examples `?siteName=building1` (single siteName
        requested) `?siteName=building1&siteName=building2&siteName=building3` (multiple siteNames requested).
    type: str
  siteType:
    description:
      - >
  ...
  deviceCategory:
    description:
      - >
        ...
        devices will be included when calculating energy consumption values, rather than specifying the list of
        returned sites. Examples `deviceCategory=Switch` (single device category requested)
  ...
  attribute:
    description:
      - >
        ...
        longitude, energyConsumed, estimatedCost, estimatedEmission, carbonIntensity, numberOfDevices If length
        of attribute list is too long, please use 'view' param instead. Examples attribute=siteHierarchy (single
        attribute requested) attribute=siteHierarchy&attribute=energyConsumed (multiple attributes requested).
    type: str
  taskId:
    description:
      - >
...
seealso:
  - name: Cisco DNA Center documentation for Sites GetSiteEnergyByID
    description: Complete reference of the GetSiteEnergyByID API.
  ...
  - name: Cisco DNA Center documentation for Sites GetSitesEnergy
    description: Complete reference of the GetSitesEnergy API.
