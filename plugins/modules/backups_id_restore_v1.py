...
DOCUMENTATION = r"""
---
module: backups_id_restore_v1
short_description: Resource module for Backups Id Restore V1
description:
  - Manage operation create of the resource Backups Id Restore V1.
  - >
    This api is used to trigger restore workflow of a specific backup. Obtain the
    `id` from the id attribute in the
    response of the `/dna/system/api/v1/backups` API. To monitor the progress and
    completion of the backup deletion ,
    please call `/dna/system/api/v1/backupRestoreExecutions/{id}` api , where id is
    the taskId attribute from the
...
EXAMPLES = r"""
- name: Create
  cisco.dnac.backups_id_restore_v1:
    dnac_host: "{{dnac_host}}"
