...
DOCUMENTATION = r"""
---
module: wired_network_devices_id_config_features_intended_layer2_feature_v1
short_description: Resource module for Wired Network Devices Id Config Features Intended
  Layer2 Feature V1
description:
  - Manage operations create, update and delete of the resource Wired Network Devices
    Id Config Features Intended Layer2 Feature V1.
  - >
    This API creates configurations for an intended feature on a wired device. Once
    all the updates to intended
    features are complete, they can be deployed to a device using the API
    /dna/intent/api/v1/networkDevices/{id}/configFeatures/intended/deploy. When the
    intended features are deployed,
    they are applied on top of the existing configurations on the device. Any existing
    configurations on the device
    which are not included in the intended features, are retained on the device.
  - >
    This API deletes the configurations for an intended feature on a wired device.
    Once all the updates to intended
    features are complete, they can be deployed to a device using the API
    /dna/intent/api/v1/networkDevices/{id}/configFeatures/intended/deploy. When the
    intended features are deployed,
    they are applied on top of the existing configurations on the device. Any existing
    ...
    which are not included in the intended features, are retained on the device.
  - >
    This API updates the configurations for an intended feature on a wired device.
    Updates to other intended features
    can be done over several iterations. Once all the updates to intended features
    are complete, they can be deployed
    to a device using the API /dna/intent/api/v1/networkDevices/{id}/configFeatures/intended/deploy.
    When the intended
    features are deployed, they are applied on top of the existing configurations
    on the device. Any existing
    configurations on the device which are not included in the intended features,
...
options:
  ...
  igmpSnoopingGlobalConfig:
    description: >-
      Wired Network Devices Id Config Features Intended Layer2 Feature's
      igmpSnoopingGlobalConfig.
    type: dict
    suboptions:
      items:
        ...
        suboptions:
          configType:
            description: >-
              Type of network functionality under a feature. Config type
              IGMP_SNOOPING_GLOBAL is for configuring global IGMP snooping settings
              on a device.
            type: str
              ...
              items:
                ...
                suboptions:
                  configType:
                    description: >-
                      Type of network functionality under a feature. Config type
                      IGMP_SNOOPING_VLAN is for configuring IGMP snooping per VLAN.
                    type: str
                  igmpSnoopingVlanMrouters:
                    description: >-
                      Wired Network Devices Id Config Features Intended Layer2
                      Feature's igmpSnoopingVlanMrouters.
                    type: dict
                    suboptions:
                      configType:
                      ...
                      items:
                        ...
                        suboptions:
                          configType:
                            description: >-
                              Type of network functionality under a feature. Config
                              type IGMP_SNOOPING_VLAN_MROUTER is for configuring IGMP
                              snooping mrouter per VLAN.
                            type: str
                          interfaceName:
                            description: >-
                              Mrouter interface name. The API
                              /dna/intent/api/v1/interface/network-dev... can be used to
                              get the list of interfaces on a device.example
                              GigabitEthernet1/0/2.
                            type: str
                  isIgmpSnoopingEnabled:
                    description: >-
                      Enable IGMP Snooping for a VLAN. Corresponding CLI - ip igmp
                      snooping vlan <1-4094>.
                    type: bool
                  isImmediateLeaveEnabled:
                    description: >-
                      Enable immediate leave processing for IGMPv2. Corresponding CLI
                      - ip igmp snooping vlan <vlan-id> immediate-leave.default false.
                    type: bool
                  isQuerierEnabled:
                    description: >-
                      Enable IGMP querier for this VLAN. Corresponding CLI - ip igmp
                      snooping vlan <vlan-id> querier.
                    type: bool
                  querierAddress:
                    description: >-
                      IGMP querier source IP address. Corresponding CLI - ip igmp
                      snooping vlan <vlan-id> querier address <ip-address>.
                    type: str
                  querierQueryInterval:
                    description: >-
                      Set the IGMP querier query interval (in seconds). Corresponding
                      CLI - ip igmp snooping vlan <vlan-id> querier query-interval
                      <1-18000>.
                    type: int
                  querierVersion:
                    description: >-
                      The switch supports IGMP version 1, IGMP version 2, and IGMP
                      version 3. These versions are interoperable on the switch. For
                      example, if IGMP snooping is enabled and the querier's version is
                      IGMPv2, and the switch receives an IGMPv3 report from a host,
                      then the switch can forward the IGMPv3 report to the multicast
                      router. Corresponding CLI - ip igmp snooping querier version
                      <1-3>. IGMP version 1 provides the basic query-response mechanism
                      that allows the multicast device to determine which multicast
                      groups are active and other processes that enable hosts to join
                      and leave a multicast group. IGMP version 2 extends IGMP
                      functionality by providing such features as the IGMP leave process
                      to reduce leave latency, group-specific queries, and an explicit
                      maximum query response time. IGMPv2 also adds the capability for
                      routers to elect the IGMP querier without depending on the
                      multicast protocol to perform this task. An IGMP version 3 switch
                      supports Basic IGMPv3SnoopingSupport (BISS), which includes support
                      for the snooping features on IGMPv1 and IGMPv2 switches and for
                      IGMPv3 membership report messages. BISS constrains the flooding of
                      multicast traffic when your network includes IGMPv3 hosts. It
                      constrains traffic to approximately the same set of ports as the
                      IGMP snooping feature on IGMPv2 or IGMPv1 hosts. An IGMPv3 switch
                      can receive messages from and forward messages to a device running
                      theSourceSpecific Multicast (SSM) feature.
                    type: str
                  vlanId:
                    description: VLAN ID.
                    type: int
          isIgmpSnoopingEnabled:
            description: >-
              Enable global IGMP Snooping. Corresponding CLI - ip igmp snooping.default
              true.
            type: bool
          isQuerierEnabled:
            description: >-
              Enable IGMP Snooping Querier. Corresponding CLI - ip igmp snooping querier.default
              false.
            type: bool
          querierAddress:
            description: >-
              Configure IGMP querier source IP address. Corresponding CLI - ip igmp
              snooping querier address <ip-address>.
            type: str
          querierQueryInterval:
            description: >-
              Set the IGMP querier query interval (in seconds). Corresponding CLI -
              ip igmp snooping querier query-interval <1-18000>.
            type: int
          querierVersion:
            description: >-
              The switch supports IGMP version 1, IGMP version 2, and IGMP version 3.
              These versions are interoperable on the switch. For example, if IGMP
              snooping is enabled and the querier's version is IGMPv2, and the switch
              receives an IGMPv3 report from a host, then the switch can forward the
              IGMPv3 report to the multicast router. Corresponding CLI - ip igmp
              snooping querier version <1-3>. IGMP version 1 provides the basic
              query-response mechanism that allows the multicast device to determine
              which multicast groups are active and other processes that enable hosts
              to join and leave a multicast group. IGMP version 2 extends IGMP
              functionality by providing such features as the IGMP leave process to
              reduce leave latency, group-specific queries, and an explicit maximum
              query response time. IGMPv2 also adds the capability for routers to
              elect the IGMP querier without depending on the multicast protocol to
              perform this task. An IGMP version 3 switch supports Basic
              IGMPv3SnoopingSupport (BISS), which includes support for the snooping
              features on IGMPv1 and IGMPv2 switches and for IGMPv3 membership report
              messages. BISS constrains the flooding of multicast traffic when your
              network includes IGMPv3 hosts. It constrains traffic to approximately
              the same set of ports as the IGMP snooping feature on IGMPv2 or IGMPv1
              hosts. An IGMPv3 switch can receive messages from and forward messages
              to a device running theSourceSpecific Multicast (SSM) feature.
            type: str
  lldpGlobalConfig:
    description: Wired Network Devices Id Config Features Intended Layer2 Feature's
      lldpGlobalConfig.
        ...
        suboptions:
          ...
          isBackboneFastEnabled:
            description: Enable BackboneFast to detect indirect link failures
              ...
              normal spanning-tree rules. Corresponding CLI - spanning-tree backbonefast.
              default false.
            type: bool
          isBpduFilterEnabled:
            description: Configure BPDU filtering. Enabling BPDU filtering on
              ...
              Corresponding CLI - spanning-tree portfast bpdufilter default.
              default false.
            type: bool
          isBpduGuardEnabled:
            description: Configure BPDU guard. Enabling BPDU filtering on PortFast
              ...
              them. Corresponding CLI - spanning-tree portfast bpduguard default.
              default false.
            type: bool
...
EXAMPLES = r"""
- name: Delete by id
  cisco.dnac.wired_network_devices_id_config_features_intended_layer2_feature_v1:
    dnac_host: "{{dnac_host}}"
