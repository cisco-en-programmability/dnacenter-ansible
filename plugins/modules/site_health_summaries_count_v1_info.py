...
DOCUMENTATION = r"""
---
module: site_health_summaries_count_v1_info
short_description: Information module for Site Health Summaries Count V1
description:
  - Get all Site Health Summaries Count V1.
  - >
    Get a count of sites. Use the available query parameters to get the count of a
    subset of sites. This API provides
    the latest data from a given `endTime` If data is not ready for the provided endTime,
    the request will fail, and
    the error message will indicate the recommended endTime to use to retrieve a complete
    data set. This behavior may
    occur if the provided endTime=currentTime, since we are not a real time system.
    When `endTime` is not provided,
    the API returns the latest data. For detailed information about the usage of the
...
- name: Get all Site Health Summaries Count V1
  cisco.dnac.site_health_summaries_count_v1_info:
    dnac_host: "{{dnac_host}}"
