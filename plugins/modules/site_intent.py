...
class DnacSite(DnacBase):
            ...
            if response and isinstance(response, dict):
                ...
                if site_updated:
                    ...
                    if site_exists:
                        ...
                        self.log("Site '{0}' created successfully".format(
                            site_name), "INFO")
    ...
    def get_diff_deleted(self, config):
        ...
        if not site_exists:
            ...
            self.log("Unable to delete site '{0}' as it's not found in Cisco Catalyst Center".format(
                site_name), "INFO")
            return self
        ...
        self.log("Received API response from 'get_membership': {0}".format(
            str(mem_response)), "DEBUG")
        site_response = mem_response.get("site").get("response")
        self.log("Site {0} response along with it's child sites: {1}".format(
            site_name, str(site_response)), "DEBUG")
        ...
        self.delete_single_site(site_id, site_name)
        self.log("The site '{0}' and its child sites have been deleted successfully".format(
            site_name), "INFO")
