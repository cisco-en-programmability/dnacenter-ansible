...
description:
  - Get all Energy Network Devices Count. - > Retrieves the total count of network devices that provide energy data, filtered
    ...
    Open API specification document - https //github.com/cisco-en-programmability/catalyst-center-api- specs/blob/main/Assurance/CE_Cat_Center_Org-deviceEnergy_1.0-1.0.1-resolved.yaml.
...
options:
  ...
  deviceCategory:
    description:
      - >
        DeviceCategory query parameter. The list of device deviceCategories. Examples `deviceCategory=Switch`
        (single device family requested) `deviceCategory=Switch&deviceCategory=Router` (multiple device
...
requirements:
  ...
  - python >= 3.12
seealso:
  - name: Cisco DNA Center documentation for Devices CountDevicesEnergy
    description: Complete reference of the CountDevicesEnergy API.
