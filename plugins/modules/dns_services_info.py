...
description:
  ...
  - Get Dns Services by id. - > Retrieves the details of the DNS Service matching the given id. For detailed information about
    ...
    DNSServices-1.0.0-resolved.yaml. - > Retrieves the list of DNS Services and offers basic filtering and sorting capabilities.
...
options:
  ...
  serverIp:
    description:
      - >
        ServerIp query parameter. IP Address of the DNS Server. This parameter supports wildcard (`*`) character
        -based search. Example `10.76.81.*` or `*56.78*` or `*50.28` Examples serverIp=10.42.3.31 (single IP
        Address is requested) serverIp=10.42.3.31&serverIp=name2&fabricVnName=name3 (multiple IP Addresses are
        requested).
    type: str
  deviceId:
    description:
  ...
  deviceSiteHierarchyId:
    description:
      - >
        ...
        ?deviceSiteHierarchyId=globalUuid/areaUuid/buildingUuid/floorUuid&deviceSiteHierarchyId=globalUuid/areaU
        uid2/buildingUuid2/floorUuid2` (multiple siteHierarchyIds requested).
    type: str
  deviceSiteId:
    description:
      - >
  ...
  ssid:
    description:
      - >
        ...
        character-based search. If the field contains the (`*`) character, please use the /query API for search.
        Ex `*Alpha*` or `Alpha*` or `*Alpha` Examples `ssid=Alpha` (single ssid requested)
        `ssid=Alpha&ssid=Guest` (multiple ssid requested).
    type: str
  id:
    description:
      - >
        Id path parameter. Unique id of the DNS Service. It is the combination of DNS Server IP (`serverIp`) and
