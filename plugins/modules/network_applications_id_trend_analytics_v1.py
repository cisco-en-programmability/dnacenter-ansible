...
DOCUMENTATION = r"""
---
module: network_applications_id_trend_analytics_v1
short_description: Resource module for Network Applications Id Trend Analytics V1
description:
  - Manage operation create of the resource Network Applications Id Trend Analytics
    V1.
  - >
    Retrieves the trend analytics of applications experience data to specific network
    application for the specified
    time range. The data will be grouped based on the given trend time interval. This
    API facilitates obtaining
    consolidated insights into the performance and status of the network applications
    over the specified start and end
    time. If startTime and endTime are not provided, the API defaults to the last
    24 hours.`siteId` and
    `trendInterval` are mandatory. `siteId` must be a site UUID of a building.For
...
EXAMPLES = r"""
- name: Create
  cisco.dnac.network_applications_id_trend_analytics_v1:
    dnac_host: "{{dnac_host}}"
