...
DOCUMENTATION = r"""
---
module: network_device_maintenance_schedules_v1_info
short_description: Information module for Network Device Maintenance Schedules V1
description:
- Get all Network Device Maintenance Schedules V1.
- >
   This API retrieves a list of scheduled maintenance windows for network devices based on filter parameters. Each
   maintenance window is composed of a start schedule and end schedule, both of which have unique identifiers
   `startId` and `endId`. These identifiers can be used to fetch the status of the start schedule and end schedule
   using the `GET /dna/intent/api/v1/activities/{id}` API. Completed maintenance schedules are automatically removed
...
EXAMPLES = r"""
- name: Get all Network Device Maintenance Schedules V1
  cisco.dnac.network_device_maintenance_schedules_v1_info:
    dnac_host: "{{dnac_host}}"
