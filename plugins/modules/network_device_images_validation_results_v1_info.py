...
DOCUMENTATION = r"""
---
module: network_device_images_validation_results_v1_info
short_description: Information module for Network Device Images Validation Results
  V1
description:
  - Get all Network Device Images Validation Results V1.
  - >
    This API provides a comprehensive overview of the outcomes from various tests
    and assessments defined by system
    and custom validations related to network device images. These results are essential
    for identifying potential
    issues, verifying configurations, and ensuring that the network meets the requirement
    for image update.
...
options:
  ...
  status:
    description:
      - Status query parameter. Status of the validation result. SUCCESS, FAILED,
        IN_PROGRESS, WARNING.
    type: str
  type:
    description:
...
requirements:
  ...
  - python >= 3.5
seealso:
  - name: Cisco DNA Center documentation for Software Image Management (SWIM) NetworkDeviceImageUpdateValidationResultsV1
    description: Complete reference of the NetworkDeviceImageUpdateValidationResultsV1
      API.
    link:
      https://developer.cisco.com/docs/dna-center/#!network-device-image-update-validation-results
notes:
  - SDK Method used are
    software_image_management_swim.SoftwareImageManagementSwim.network_device_image_update_validation_results_v1,
  - Paths used are get /dna/intent/api/v1/networkDeviceImages/validationResults,
"""
EXAMPLES = r"""
- name: Get all Network Device Images Validation Results V1
  cisco.dnac.network_device_images_validation_results_v1_info:
    dnac_host: "{{dnac_host}}"
