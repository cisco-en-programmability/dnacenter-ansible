...
class DnacDevice(DnacBase):
    ...
    def handle_all_provisioned(self, device_type):
        ...
        self.status = "success"
        self.result['changed'] = True
        self.log("All {0} Devices provisioned successfully!!".format(
            device_type), "INFO")
    ...
    def provisioned_wired_device(self):
        ...
        for prov_dict in provision_wired_list:
            ...
            # This resync retry interval will be in seconds which will check
            # device status at given interval
            ...
            while resync_retry_count:
                ...
                self.log(
                    "Device is in {0} state waiting for Managed State.".format(
                        response['managementState']), "DEBUG")
            ...
            try:
                ...
                if response.get("status") == "failed":
                    ...
                    error_msg = "Cannot do Provisioning for device {0} beacuse of {1}".format(
    ...
    def get_provision_wired_device(self, device_ip):
        ...
        try:
            ...
            if response.get("status") == "success" and "Wired Provisioned device detail retrieved successfully." in response.get(
                    "description"):
                flag = 2
    ...
    def get_diff_merged(self, config):
                    ...
                    while True:
                        ...
                        if '/task/' in execution_details.get("progress"):
                            ...
                            failure_reason = execution_details.get(
                                "failureReason")
                            if failure_reason:
                                self.msg = "Device addition get failed because of {0}".format(
                                    failure_reason)
                            else:
        ...
        if credential_update:
            ...
            password = "Testing@123"
            export_payload = {"deviceUuids": device_uuids,
                              "password": password, "operationEnum": "0"}
